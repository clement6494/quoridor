...
y+=32;}
/****************************************************************************/
distance=abs(x - player->lastx)+abs(y - player->lasty);    //calcul de la distance parcouru
/********************************/
if (distance==0){                                         // aucun deplacement
        printf("vous etes deja sur cette case");
    bouge(tab[17][17], t_joueur player  );
}
/****************************/
if (distance>2){
...
if (distance==2){                                      /* on teste si on enjambe bien un joueur et si deplacement possible
        (enjambe se fait par joueur_sur_case(tab, (x+player->lastx)/2, (y+player->lasty)/2)?*/
    if ((joueur_sur_case(tab, (x+player->lastx)/2, (y+player->lasty)/2)==1)&&(barrière_bloque(tab, x , y )==0)&&(joueur_sur_case(tab, x , y )==0)){
       ...
       tabn[x][y-48]=tabn[lastx][lasty];               //on sauvegarde ce tour
        ...
        player->lastx=x;                                   //on enregistre la nouvelle position du joueur
        player->lasty=y;
    else {
